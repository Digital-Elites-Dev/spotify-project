...
access_token = None
access_token_expiry = 0  # Expiry time in seconds

def get_spotify_token():
    global access_token, access_token_expiry
    
    # If token is expired or doesn't exist, get a new token
    if access_token is None or time() >= access_token_expiry:
        token_url = "https://accounts.spotify.com/api/token"
        client_id = settings.SPOTIFY_CLIENT_ID
        client_secret = settings.SPOTIFY_CLIENT_SECRET
        
        # Data matching the curl command, including client_id and client_secret
        data = {
        ...
        # Sending the POST request to the Spotify token endpoint
        response = requests.post(token_url, data=data, headers=headers)
        
        if response.status_code == 200:
            token_data = response.json()
            access_token = token_data['access_token']
            access_token_expiry = time() + token_data['expires_in']  # Set expiry time
            print(f"New token: {access_token}")  # For debugging
            return access_token
        else:
...
#     search_url = f'https://api.spotify.com/v1/search?q={search_query}&type=track&market=ES&limit=10&offset=5&include_external=audio'
#     headers = {'Authorization': f'Bearer {token}'}
...
#             # Save each track to the database, linking to the created playlist
#             for track in tracks:
#                 Track.objects.update_or_create(
#                     spotify_id=track['id'],
#                     playlist=playlist,
...
#             # Step 6: Return the newly created playlist information
...
#         try:
#             # Step 1: Retrieve a valid Spotify access token
#             user_spotify_token = get_spotify_token()
#             print("Token is Here:")
#             print(user_spotify_token)
#             # Extract track and artist names from the request
#             track_name = request.POST.get('track_name', '')
...
#             # Step 2: Get the user ID (replace with actual logic to retrieve the user ID if necessary)
#             user_id = get_user_id_from_token(user_spotify_token)  # Implement this function to fetch user ID
#             print("My ID: ")
#             print(user_id)
